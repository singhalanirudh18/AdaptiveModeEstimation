...
cdef find_knn(UTYPE_t num_iter, DTYPE_t[:] Y, UTYPE_t k, DTYPE_t delta, str which_beta, DTYPE_t beta_c, DTYPE_t sigma, DTYPE_t[:] ds_input, UTYPE_t[:] ts_input):
    ...
    cdef UTYPE_t i, j
    for i in range(n):
        if(ts[i] == 0): #get a new sample if the point is not sampled before
            ds[i] = np.random.normal(Y[i],sigma)
            ts[i] = 1
    ...
    j = ds_sort_idx[k-1]
    cdef DTYPE_t[:] kth_element_parametrs = np.zeros(3)
    kth_element_parametrs[0] = ds[j]
    kth_element_parametrs[1] = ds[j] + beta(ts[j], delta, n, beta_c) #stores the UCB
    kth_element_parametrs[2] = ds[j] - beta(ts[j], delta, n, beta_c) #stores the LCB
    cdef pair[DTYPE_t[:],UTYPE_t] b = pair[DTYPE_t[:],UTYPE_t] (kth_element_parametrs,j) #stores the k^th element, its distance, its UCB and its LCB
    ...
    for t in range(num_iter):
        ...
        if a1.first >= b.first[2] : #UCB[a1]>=LCB[b]
            knn_found = False
            i = a1.second
            beta_i = update_estimate_get_confidence(Y, ds, ts, i, delta, beta, beta_c, sigma)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef inline DTYPE_t update_estimate_get_confidence(DTYPE_t[:] Y, DTYPE_t[:] ds, UTYPE_t[:] ts, UTYPE_t i, DTYPE_t delta, CONFIDENCE_BOUND_FUN_t beta, DTYPE_t beta_c, DTYPE_t sigma):
    cdef UTYPE_t n = Y.shape[0]
    ts[i] += 1
    ds[i] = (ds[i] * (ts[i] - 1)) / ts[i] + (np.random.normal(Y[i],sigma)) / ts[i]
...
class FindKNN(object):
    ...
    def __init__(self, Y, k, delta=1e-6, which_beta='theoretical', beta_c=0.75, sigma = 0.25):
    ...
    def query(self, num_iter, ds= None, ts= None):
        ds_out,ts_out, m, knn_found = find_knn(num_iter, self.Y, self.k, self.delta, self.which_beta, self.beta_c, self.sigma, ds, ts)
        return ds_out,ts_out, m, knn_found
